...
filtered_data_except = FILTER data BY ((SUBSTRING($0#'created_at', 4, 7) != 'Jun') OR (SUBSTRING($0#'created_at', 4, 7) != 'Jul'));
...
filtered_data_jul = FILTER data BY ((SUBSTRING($0#'created_at', 4, 7) == 'Jul') AND (((int)SUBSTRING($0#'created_at', 8, 10) != 4) AND ((int)SUBSTRING($0#'created_at', 8, 10) != 5)
			AND ((int)SUBSTRING($0#'created_at', 8, 10) != 8) AND ((int)SUBSTRING($0#'created_at', 8, 10) != 9)
			AND ((int)SUBSTRING($0#'created_at', 8, 10) != 13)));

filtered_data = UNION filtered_data_jun, filtered_data_jul, filtered_data_except;
...
Store big_groups INTO 'word_count_not_during_cup';
